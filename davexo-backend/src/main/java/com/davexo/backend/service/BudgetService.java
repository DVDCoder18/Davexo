...
public class BudgetService {
    ...
    @Transactional
    public BudgetResponseDto createBudget(BudgetRequestDto budgetRequestDto, User authenticatedUser) {
    ...
    public BudgetResponseDto updateBudget(
        ...
        List<ExpenseCategory> currentFollowedCategories = expenseCategoryRepository.findAllByBudgetIdAndUserId(
                budgetId,
                userId);
        ...
        if (newScope == BudgetScope.SELECTED_CATEGORIES) {
            ...
            for (ExpenseCategory expenseCategory : newFollowedCategories) {
                ...
                // A category already linked to another specific budget cannot be reassigned
                // implicitly.
                if (expenseCategory.getBudget() != null
    ...
    @Transactional
    public BudgetResponseDto getBudgetDetail(Integer budgetId, Integer userId) {
    ...
    public List<BudgetResponseDto> getAllBudgets(Integer userId) {
        ...
        List<ExpenseCategory> followedCategories = expenseCategoryRepository.findAllByBudgetIdInAndUserId(
                budgetIds,
                userId);
                ...
                .map(budget -> {
                    ...
                    List<Integer> followedCategoryIds = followedCategories.stream()
                            .filter(category -> category.getBudget() != null
                                    && category.getBudget().getId().equals(budget.getId()))
                            .map(ExpenseCategory::getId)
                            .toList();
